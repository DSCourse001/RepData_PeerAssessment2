...
## Data Processing
Data came in csv file which is compressed in bz2 format. Unfortunately that file cannot be loaded using "fast" loading tools like ```fread``` because of End-Of-Line character used in remarks. 
As we will see data is skewed and there should be done additional processing to make data understandable for machine language.

After formatting the data we will produce few data sets with summary data to answer on question.
...
### Top 5 Events by Damage to Property and Corp
